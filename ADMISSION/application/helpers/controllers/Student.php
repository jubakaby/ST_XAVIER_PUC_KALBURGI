...
class Student extends BaseController
{
    ...
    public function viewPersonalDetail() {
        ...
        if($appStatus->admission_status == 1 || $appStatus->joined_status == 1){
            redirect('dashboard');
        }else{
    ...
    public function viewSchoolDetail() {
        ...
        $data['boardInfo'] = $this->student_model->getBoardNameById($this->sslc_board_name_id);
        $data['documentInfo'] = $this->student_model->getDocumnetDetails($this->student_row_id);
        $data['studentApplicationInfo'] = $student;
        $_SESSION['application_number_status'] = false;
        if(!empty($student)){
    ...
    function getStudentMarkSheet(){
        ...
        if($this->sslc_board_name_id == 1){
            ...
            // }
        } else if($this->sslc_board_name_id == 2){
            $this->load->view('student_sslc_subjects/cbse_subjects');
        }else if($this->sslc_board_name_id == 3){
            $this->load->view('student_sslc_subjects/icse_board');
        }else if($this->sslc_board_name_id == 4){
    ...
    function saveStudentPersonalInfo(){
        ...
        if($this->form_validation->run() == FALSE) {
            ...
            if($this->upload->do_upload('pastor_file')) {
                ...
                if($data['file_size'] > 102){
                    $this->session->set_flashdata('error', 'File Maximum size of 100KB');
                    redirect('viewPersonalDetail');  
                } else{
        ...
        if($this->form_validation->run() == FALSE) {
            ...
            if($retun_id > 0){
                $studentPersonalInfo = array(
                    ...
                    $retun = $this->student_model->updateStudentPersonalInfo($this->student_row_id,$studentPersonalInfo); 
            ...
            if($retun_id > 0) {
                ...
                $this->session->set_flashdata('error', 'Failed to Add SchoolInfo');
            }
        }
    }
    ...
    function studentFinalSubmission(){
        ...
        // } else {
            ...
            $applicationNumber = '20'.sprintf('%04d', $this->student_row_id);
    ...
    function payTmPaymentResponse(){
        ...
        $this->global['pageTitle'] = ''.TAB_TITLE.' : PayTm Payment Response';
        $this->loadViews("admission/payment", $this->global , $data ,NULL);
