...
class Admission extends BaseController
{
    ...
    public function admissionFeeProcess(){
        ...
        $checkSum = "";
        $paramList = array();
        $remarks = 'SJPUC I PUC Fee Payment for Admission 2020-21';
        ...
        $CUST_ID = "SJPUC".$studentInfo->application_number;
        $INDUSTRY_TYPE_ID = "Retail";
        $CHANNEL_ID = "WEB";
        $TXN_AMOUNT = $fee_amount;
        if($response > 0){
            $ORDER_ID = 'SJPUCFEE'.$response;
    ...
    public function payTmfeePaymentResponse(){
       ...
       if(empty($responseStatus->receipt_number) && $isValidChecksum == true){
        if($_POST['STATUS'] == 'TXN_SUCCESS'){
            ...
            $row_id = $this->admission_model->addFeeDetailsInfo($overallFee);
            ...
            foreach($feeStructureInfo as $fee){
                if($filter['board_name'] == 'SSLC'){ 
                  ...
                  }else if($filter['board_name'] == 'OTHER'){
                     $fee_structure_amt = $fee->fee_amount_icse_cbse;
                  }else{
                     $fee_structure_amt = $fee->fee_amount_nri;
                  }
                $fee_structure = $this->admission_model->checkFeeStructurePaidExist($studentInfo->application_number,$fee->row_id);
       ...
       if(empty($responseStatus->receipt_number)){
        if($_POST['STATUS'] == 'TXN_SUCCESS'){
            ...
            foreach($feeStructureInfo as $fee){
                if($filter['board_name'] == 'SSLC'){ 
                    $fee_structure_amt = $fee->fee_amount_state_board;
                  }else if($filter['board_name'] == 'ICSE' || $filter['board_name'] == 'CBSE'){
                     $fee_structure_amt = $fee->fee_amount_icse_cbse;
                  }else{
                     $fee_structure_amt = $fee->fee_amount_nri;
                  }
