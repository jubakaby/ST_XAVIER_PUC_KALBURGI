...
class Leave_model extends CI_Model
{
    ...
    public function getAllStaffLeaveInfo()
    {
        ...
        shift.name as shift_name, shift.shift_code, shift.start_time, shift.end_time, 
...
public function checkLeaveAppliedAlready($date_from,$staff_id)
{
    ...
    return $query->row();
       ...
       function getStaffLeaveInfoByRow_Id($row_id){
        $this->db->select('leave.date_from, leave.date_to, leave.leave_reason, leave.total_days_leave,leave.row_id, leave.approved_status, leave.leave_type, leave.medical_certificate, leave.approved_by, leave.rejected_by, shift.name as shift_name, shift.shift_code, shift.start_time, shift.end_time, staff.type, staff.staff_id, staff.email, staff.name,dept.name as department, staff.mobile_one, Role.role, staff.address');
        $this->db->from('tbl_staff_applied_leave as leave'); 
        $this->db->join('tbl_staff as staff', 'staff.staff_id = leave.staff_id','left');
        $this->db->join('tbl_roles as Role', 'Role.roleId = staff.role','left');
        $this->db->join('tbl_department as dept', 'dept.dept_id = staff.department_id','left');
...
//update leave info
    ...
    public function getStaffAppliedLeaveInfoByDate($todayDate, $staff_id){
        $this->db->select('leave.date_from, leave.date_to, leave.leave_reason, leave.total_days_leave,
        leave.row_id,leave.approved_status, leave.leave_type,
        staff.type, staff.staff_id, staff.email, staff.name,dept.name as department, staff.mobile_one, Role.role, staff.address');
        $this->db->from('tbl_staff_applied_leave as leave'); 
        $this->db->join('tbl_staff as staff', 'staff.staff_id = leave.staff_id');
        $this->db->join('tbl_roles as Role', 'Role.roleId = staff.role');
