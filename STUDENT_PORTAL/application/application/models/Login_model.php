...

{
    ...
    function loginMe($student_id,$password,$term_name)

    {

        $this->db->join('tbl_students_info as std','std.student_id = register.student_id');

        $this->db->select('std.row_id,register.student_id,register.password,register.dob,std.student_name,std.term_name,std.section_name,std.stream_name');

        $this->db->from('tbl_student_app_registration as register');

        $this->db->where('register.student_id', $student_id);
